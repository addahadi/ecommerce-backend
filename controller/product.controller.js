...
function filterProduct(req , res){
    const {category , price , sort , star , region , pages} = req.query;
    ...
    FROM product p, product_img i, (SELECT productId, rating FROM product_rating ) r 
    WHERE p.productId = i.productId AND p.productId = r.productId
`;    
